...
class NodeModel(torch.nn.Module):
    def __init__(self, edge_feature_size, node_feature_size, layer_size):
        ...
        self.node_mlp_1 = torch.nn.Sequential(
            torch.nn.Linear(node_feature_size + edge_feature_size, layer_size),
            torch.nn.BatchNorm1d(layer_size),
            torch.nn.ReLU()
        )
        self.node_mlp_2 = torch.nn.Sequential(
            torch.nn.Linear(node_feature_size + layer_size, node_feature_size + layer_size),
            torch.nn.BatchNorm1d(layer_size + node_feature_size),
            torch.nn.ReLU()
        )

        self.linear = torch.nn.Linear(layer_size + node_feature_size, node_feature_size)
...
class SJN_Meta(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, u, batch):
        ...
        edge_attr = F.dropout(edge_attr, p=0.1, training=self.training)
